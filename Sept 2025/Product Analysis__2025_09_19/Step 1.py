...
where lower(web_description) like "%" || '{product}' || "%"
""")

filter_items.display()

# COMMAND ----------

# MAGIC %md
# MAGIC ## Step 1.1 => big numbers
...
where lower(web_description) = '{product}'
...
# MAGIC %md
# MAGIC
# MAGIC ## Step 2 : blueprints tables

# COMMAND ----------

sku_analysis = spark.sql(f"""
select *
FROM {sandbox_schema}.{demo_name}_sku_analysis
where lower(web_description) like "%" || '{product}' || "%"
""")
...
select *
FROM {sandbox_schema}.{demo_name}_sku_time_series
where lower(web_description) like "%" || '{product}' || "%"
""")
...
# MAGIC %md
# MAGIC
# MAGIC ## Step 3: applying some logics
...
--- basic table with filters
  ...
  where
    1=1
    and display_interval = "Year-To-Date"
),
...
with source as (
    ...
    *
    FROM {sandbox_schema}.{demo_name}_sku_time_series
    where lower(web_description) like "%" || '{product}' || "%"
    and period_month >= "2024-08-01"
    and lower(web_description) = '{product}'
),

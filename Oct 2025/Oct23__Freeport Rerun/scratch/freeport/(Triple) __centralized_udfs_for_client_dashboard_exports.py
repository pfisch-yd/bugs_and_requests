...
def run_market_share (demo_name):
    df = get_default_gsheet_client().read_sheet(
        "1F8bvKrBdlY0GIm9xAU5v3IM7ERLM6kUdi_wYph9JJds",
        1374540499
    )
...
def run_testing_tariffs (demo_name):
    df = get_default_gsheet_client().read_sheet(
        "1F8bvKrBdlY0GIm9xAU5v3IM7ERLM6kUdi_wYph9JJds",
        1374540499
    )
...
def run_dash_metric_save(demo_name):
    df = get_default_gsheet_client().read_sheet(
        "1F8bvKrBdlY0GIm9xAU5v3IM7ERLM6kUdi_wYph9JJds",
        1374540499
    )
...
def run_hot_fix (demo_name):
    ...
    if market_share:
        print("internally run_export_product_analysis_module")
        run_export_product_analysis_module(
            sandbox_schema,
            prod_schema,
            demo_name,
...
def bulk_run_everything (demo_name):
    # Step 1: Read as Pandas (make sure the file is accessible)
    pdf = pd.read_csv("/Workspace/Corporate Sensitive - Dashboard Templates/Brands Dashboard Templates/clients.csv")

    # Step 2: Convert to Spark
    df = spark.createDataFrame(pdf)

    # Step 3: Rename confusing column
    df = df.withColumnRenamed(
        "demo_name (lower case, no space, no version e.g..: triplecrowncentral)",
        "demo_name"
    ).withColumnRenamed(
        "Timestamp", "timestamp"
    )
...
def hot_fix_for_amazon (demo_name):
    # Step 1: Read as Pandas (make sure the file is accessible)
    pdf = pd.read_csv("/Workspace/Corporate Sensitive - Dashboard Templates/Brands Dashboard Templates/clients.csv")
...
def fix_for_glossier (demo_name):
    df = get_default_gsheet_client().read_sheet(
        "1F8bvKrBdlY0GIm9xAU5v3IM7ERLM6kUdi_wYph9JJds",
        1374540499
    )
...
def run_client_specs (demo_name, parent_brand):
    df = get_default_gsheet_client().read_sheet(
        "1F8bvKrBdlY0GIm9xAU5v3IM7ERLM6kUdi_wYph9JJds",
        1374540499
    )
...
def run_everything_b (demo_name):
    df = get_default_gsheet_client().read_sheet(
        "1F8bvKrBdlY0GIm9xAU5v3IM7ERLM6kUdi_wYph9JJds",
        1374540499
    )
...
run_everything_b("triplecrowncentral")

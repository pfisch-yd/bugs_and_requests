...
def freeport_module(sandbox_schema, prod_schema, demo_name, module_type, use_sampling=False, sample_fraction=0.01, column=None):
    ...
    input_table = spark.sql(query_view)
    input_table.display()

    output_table = spark.sql(f"select * from {fp_catalog}.{schema}.{demo_name}{table_suffix}1d")
    ...
    view_table = spark.sql(f"select * from {fp_catalog}.{schema}.{demo_name}{table_suffix}{view_suffix}")
    view_table.display()
...
input_table = spark.sql(f"select * from {input_tables_name}")
input_table.display()
...
output_table = spark.sql(f"select * from {fp_catalog}.{schema}.{demo_name}{table_suffix}1c")
output_table.display()

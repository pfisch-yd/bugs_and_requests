...
def freeport_sku_detail(sandbox_schema, prod_schema, demo_name):
    table_suffix = "_sku_detail"
    table_nickname = "sdet"

    df = spark.sql(f"""
        SELECT * except (ASP) FROM {prod_schema}.{demo_name}{table_suffix}
    """)
...
try:
    ...
    freeport_sku_detail(sandbox_schema, prod_schema, demo_name)
except:

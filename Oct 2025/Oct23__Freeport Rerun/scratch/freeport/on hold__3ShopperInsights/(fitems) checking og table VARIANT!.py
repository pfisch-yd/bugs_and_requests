...
sku_detail = spark.sql(f"""
    SELECT product_attributes FROM ydx_shiseido_analysts_silver.items_final
    where product_attributes is not null
    limit 2500
""")

#create_table("ydx_internal_analysts_gold", 'lixil_v38_filter_items', sku_detail, overwrite=True)
sku_detail.display()
...
sku_detail = spark.sql(f"""
    ...
    SELECT
        user_id,
        sum(gmv) as g
    FROM ydx_shiseido_analysts_gold.shiseido_v38_filter_items
    group by user_id
    ...
    select
    ff.*
    from ydx_shiseido_analysts_gold.shiseido_v38_filter_items as ff
    inner join userss
    on ff.user_id = userss.user_id
""")
...
# create_table("ydx_internal_analysts_gold", 'shiseido_v38_filter_items', sku_detail, overwrite=True)

# COMMAND ----------

df = spark.sql(f"""
    SELECT product_attributes FROM {prod_schema}.{demo_name}{table_suffix}
    limit 10
""")

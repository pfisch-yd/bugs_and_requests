...
# MAGIC %md
# MAGIC
# MAGIC [UNRESOLVED_COLUMN.WITH_SUGGESTION] A column, variable, or function parameter with name `b`.`user_id` cannot be resolved. Did you mean one of the following? [`a`.`user_id`, `a`.`item_id`, `a`.`order_id`, `a`.`row_id`, `b`.`year`]. SQLSTATE: 42703
...
# MAGIC
# MAGIC     FROM yd_sensitive_corporate.ydx_internal_analysts_sandbox.testesteelauder_v38_filter_items a
...
with rolling_user_count as (
    SELECT count(distinct user_id) as rolling_active_users
    FROM yd_sensitive_corporate.ydx_retail_silver.leia_active_users_2yr_222
    ),

    annual_user_count as (
    SELECT year, count(distinct user_id) as annual_active_users
    FROM yd_sensitive_corporate.ydx_retail_silver.leia_annual_active_users
    GROUP BY 1
    ),
    ...
    FROM yd_sensitive_corporate.ydx_internal_analysts_sandbox.testesteelauder_v38_filter_items a
    LEFT join yd_sensitive_corporate.ydx_retail_silver.leia_annual_active_users b ON a.user_id = b.user_id and a.year = b.year
    LEFT JOIN rolling_user_count c
    LEFT JOIN annual_user_count d ON a.year = d.year
    )
    ...
    select a.*, b.hh_size_adj_factor as age_adj_2yr, c.hh_size_adj_factor as age_adj_annual
    FROM users a
    LEFT JOIN yd_sensitive_corporate.ydx_retail_silver.generation_hh_size_adj_2yr b on LEFT(a.year, 4) = b.year and a.age_cohort = b.age_cohort
    LEFT JOIN yd_sensitive_corporate.ydx_retail_silver.generation_hh_size_adj_annual c on LEFT(a.year, 4) = c.year and a.age_cohort = c.age_cohort
""")
...
deliverable = get_or_create_deliverable(
    ...
    output_table=fp_catalog+"."+prod_schema + "."+demo_name+'_filter_items__20250922c',

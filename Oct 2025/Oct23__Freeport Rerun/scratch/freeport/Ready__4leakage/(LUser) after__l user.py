...
def freeport_leakage_users(sandbox_schema, prod_schema, demo_name):
    df = spark.sql(f"""
        SELECT * FROM {prod_schema}.{demo_name}_leakage_users
    """)
    ...
    sources = [
        {
            ...
            'table_name': demo_name+"_leakage_users",
    ...
    module_name = "pfisch__luser_20250923b"

    # Create display interval deliverable
    query_template = get_or_create_query_template(
        slug=module_name,
        query_string=query_string,
        template_description="pfisch test luser",
        version_description="pfisch test luser",
    )
    ...
    deliverable = get_or_create_deliverable(
        ...
        input_tables=[
                f"{ss_catalog}.{prod_schema}.{demo_name}_leakage_users"
            ],
        output_table=fp_catalog+"."+prod_schema + "."+demo_name+'_leakage_users2',
        query_parameters={"columns": accepted_columns},
        description="test luser 20250923b",
...
try:
    ...
    freeport_leakage_users(sandbox_schema, prod_schema, demo_name)
except:
    print("failed for {}".format(demo_name))

  ...
  select
    array_join(
      array_compact(
        transform(
          array_sort(
            array_agg(distinct struct(clean_brand as s0, clean_brand as target)),
            (l, r) -> (
              case
                when (l)['s0'] is null and (r)['s0'] is null then 0
...
filtered_data as (
  ...
  where
    quarter_rank <= 17
    and channel = {{Channel-5}}
    and quarter in {{Quarter-1}}
),
...
select
  brand_name,
  brand_total_gmv as `sort-c3He2yXaNj-0`,
  quarter,
